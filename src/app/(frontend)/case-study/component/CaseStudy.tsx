...
const CaseStudy = ({
  ...
  const currentWorkIndex = WorkData.findIndex((work) => work.link === path);
  const nextWork = WorkData[(currentWorkIndex + 1) % WorkData.length];
  ...
  return (
    <TransitionContainer initial={100} exit={-400}>
      <div>
        ...
        <div className="w-main mx-auto mb-40 md:mb-60">
          <div className="relative rounded-lg overflow-hidden">
            <div className="flex justify-center gap-5 lg:gap-20 sm:items-center p-5 sm:p-10 lg:p-20 bg-foreground/3 relative z-10 flex-col sm:flex-row">
              <div>
                <h3 className="uppercase md:text-xl text-foreground/55 tracking-wider">
                  Next project
                </h3>
                <TransitionLink
                  href={nextWork.link}
                ...
                >
                  <h2 className="text-4xl md:text-5xl lg:text-6xl xl:text-8xl xl:-ml-2 pb-2 border-b-2  border-dashed border-b-transparent duration-300 group-hover:border-b-foreground/30">
                    {nextWork.title}
                  </h2>
                  <GoChevronRight className="text-foreground/20 size-10 lg:size-14 xl:size-20" />
                </TransitionLink>
              </div>
              <TransitionLink
                href={nextWork.link}
            ...
            <motion.div
              ...
              className="absolute top-1/2 left-1/2 -translate-1/2 opacity-0 w-full"
            >
              <Image
                src={nextWork.image}
                alt={nextWork.title}
                width={1000}
                height={1000}
                className="w-full h-auto aspect-square blur-xl"
              />
            </motion.div>
          </div>
        </div>
