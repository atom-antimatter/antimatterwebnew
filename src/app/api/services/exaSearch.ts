...
const getExaClient = () => {
  const key = process.env.EXA_API_KEY || process.env.EXASEARCH_API_KEY;
  if (!key) {
    throw new Error("EXA API key must be provided via EXA_API_KEY or EXASEARCH_API_KEY");
  }
...
export const exaSearch = async (
  ...
  try {
    ...
    const searchResponse = await getExaClient().searchAndContents(query, {
      numResults,
