...
const AtlasMap = forwardRef<AtlasMapRef, AtlasMapProps>(
  ...
  useEffect(() => {
    ...
    handler.setInputAction((ev: Cesium.ScreenSpaceEventHandler.PositionedEvent) => {
      ...
      if (picked?.id?.dcData)     { onSelectDc?.(picked.id.dcData as DataCenter); onSelectLinode?.(null); }
      else if (picked?.id?.linodeData) { onSelectLinode?.(picked.id.linodeData as ProviderRegion); onSelectDc?.(null); setTooltip(null); }
      else {
        ...
        if (onMapClick) {
          ...
          if (cart) { const c = Cesium.Cartographic.fromCartesian(cart); onMapClick(Cesium.Math.toDegrees(c.latitude), Cesium.Math.toDegrees(c.longitude)); }
        }
      }
